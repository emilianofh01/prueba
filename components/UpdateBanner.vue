    ...
    methods: {
        listenUpdate() {
            ...
            channel.addEventListener('message', ({ data }) => {
                ...
                let container = document.querySelector('.reload_request_container'),
                    banner = container.querySelector('.reload_request');

                container.style.setProperty('--height', `${banner.clientHeight}px`)
                this.update = true;
            })
        }
    },
...
<template>
    <div :class="`reload_request_container ${update ? 'show' : ''}`">
        <div class="reload_request">
            <div class="reload_request-right_container">
                ...
                <div class="right_container-description_container">
                    <h2 class="description_container-title">
                        ¡Actualización disponible!
                    </h2>
                    <p class="description_container-msg">
                        La actualización se aplicará automaticamente al navegar dentro de la app o si lo prefieres,
                        recarga la app.
                    </p>
                </div>
            </div>

            <button class="right_container-button">Aplicar actualización</button>
        </div>
    </div>
</template>